...
ORG 04h
    ...
    ISR:
      ...
      BTFSC ((PIR1) and 07Fh), 0 ; verifica si se desbordo el timer0
      CALL SUB_TM1 ; llama a la subrrutina de interrupcion del tiemer 0
      BCF ((PIR1) and 07Fh), 0 ; Se limpia la bandera activada.
...
main:
 ...
 ; subrutinas de cofiguracion
    ...
    BANKSEL PIE1
    BSF ((PIE1) and 07Fh), 0 ; Se activa la interrupcion del timer1
    BANKSEL PIR1
    BCF ((PIR1) and 07Fh), 0 ; Se limpia la bandera del timer1
    BANKSEL T1CON
    BSF ((T1CON) and 07Fh), 0 ; Se activa el timer1
    BCF ((T1CON) and 07Fh), 1 ; Reloj interno
    BSF ((T1CON) and 07Fh), 4
    BSF ((T1CON) and 07Fh), 5 ; pre-scaler de 1:8
...
PULL_UP:
    ...
    BCF ((OPTION_REG) and 07Fh), 5 ; Se establece que se usara oscilador interno
    BCF ((OPTION_REG) and 07Fh), 3 ; el prescaler se asigna al timer 0
    BSF ((OPTION_REG) and 07Fh), 2
    BSF ((OPTION_REG) and 07Fh), 1
    BSF ((OPTION_REG) and 07Fh), 0 ; el prescaler es de 256
    BANKSEL WPUB
    BSF WPUB,0
    BSF WPUB,1
    BSF WPUB,2
...
CONF_INTCON:
    BANKSEL INTCON
    BSF ((INTCON) and 07Fh), 7 ; Se activan las interrupciones globales
    BCF ((INTCON) and 07Fh), 0 ; Se colaca la bandera en 0 por precaucion
    BSF ((INTCON) and 07Fh), 3 ; Permite interrupciones en el puerto B
    BSF ((INTCON) and 07Fh), 5 ; Permite interrupion del timer 0
    BCF ((INTCON) and 07Fh), 2 ; limpia bandera de desbordamiento de timer 0
    RETURN
